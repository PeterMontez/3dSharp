...
{
    public List<FloorTriangle> Triangles { get; set; } = new List<FloorTriangle>();

    public List<Triangle2d> Rendered { get; set; } = new List<Triangle2d>();

    public FloorRenderer(List<FloorTriangle> triangles)
    {
        Triangles = triangles;
    }
    ...
    public List<Triangle2d> Render(Camera camera)
    {
        ...
        foreach (FloorTriangle triangle in Triangles)
        {
            if (!RenderCheck(triangle, plane, RenderLogic))
                continue;
    ...
    public bool RenderCheck(FloorTriangle triangle, Plane plane, byte direction)
    {
        foreach (var point in triangle.points)
        {
            if (plane.X*point.X + plane.Y*point.Y + plane.Z*point.Z - plane.result > 0 && direction == 1)
